...
RESULT_COLS: "list[str]" = [
    ...
    "version",
    "locale",
    "split",  # All together, these point to a dataset split
...
def handle_split(fpath: str) -> dict[str, Any]:
    ...
    def _get_row_total(pt: pd.DataFrame, lbl: str) -> int:
        return (
            int(pd.to_numeric(pt.loc[lbl, "TOTAL"]))
            if lbl in list(pt.index.values)
            else int(0)
        )

    def _get_col_total(pt: pd.DataFrame, lbl: str) -> int:
        return (
            int(pd.to_numeric(pt.loc["TOTAL", lbl]))
            if lbl in list(pt.columns.values)
            else int(0)
        )
    ...
    # Replace NA with NODATA - Changed to remove the FutureError warnings
    df.fillna(value=NODATA, inplace=True)
    # for col in df:
    #     dtyp = df[col].dtype
    #     if dtyp in (int, float):
    ...
    res = {
        ...
        "duplicate_ratio": float(
            "{:.4f}".format(duplicate_sentence_cnt / unique_sentences)
        ),
        "duplicate_ratio_lower": float(
            "{:.4f}".format(duplicate_sentence_cnt_lower / unique_sentences_lower)
        ),
        ...
        "genders_f_m_ratio": float("{:.4f}".format(_females / _males))
        if _males > 0
        else -1,
    }
    return res
...
def main() -> None:
    ...
    for sp in SPLITS_IDS:
        tsv_paths.extend(
            glob.glob(os.path.join(experiments_dir, "**", sp + ".tsv"), recursive=True)
        )
    tsv_paths.sort()
    ...
    with mp.Pool(PROC_COUNT) as pool:
        results: list[dict[str, Any]] = pool.map(handle_split, tsv_paths)
    ...
    print("\n" + "-" * 80)
    print(
        f'Finished processing of {cnt_total} splits in {str(process_timedelta)} secs, avg duration {float("{:.3f}".format(avg_seconds))} secs'
    )
