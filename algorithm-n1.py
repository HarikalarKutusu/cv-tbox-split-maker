...
def corpora_creator_v2(pth: str):
    ...
    def _check_intersection(df1: pd.DataFrame, df2: pd.DataFrame, df1_name: str, df2_name: str) -> None:
        _intersect_df: pd.DataFrame = pd.merge(df1, df2, how='inner')
        if _intersect_df.shape[0] > 0:
            print(f'!!! ERROR IN ALGORITHM, SPLIT INTERSECTION FOUND - {df1_name} vs {df2_name}')
    ...
    sentences_df: pd.DataFrame = validated_df.groupby('sentence_lower').agg( {'path': 'count'} ).reset_index()    # get kst with count agg
    sentences_df.rename(columns= {'path': 'recorded_count'}, inplace=True )                                       # rename agg column
    sentences_df.sort_values(by='recorded_count', ascending=True, inplace=True)                                   # sort in ascending recorded count
    sentences_df['cumulative'] = sentences_df['recorded_count'].cumsum()                                          # add a cumulative sum for easy access
    ...
    if VERBOSE:
        print(f'>>> Processing - {total_validated} validated records with {total_sentences} lower-case unique sentences.')
        print(f'>>> Targeting - Train: {TRAIN_PERCENTAGE}%=>{train_target} recs, Dev: {DEV_PERCENTAGE}%=>{dev_target} recs, Test: {TEST_PERCENTAGE}%=>{test_target} recs ')
        # print()

    # Test
    _slice: pd.DataFrame = sentences_df[ sentences_df['cumulative'].astype(int) <= test_target ]    # use cumulative column to get list of sentences to match the amount
    _sentences: list[str] = _slice['sentence_lower'].to_list()                                      # convert to list
    test_df: pd.DataFrame = validated_df[ validated_df['sentence_lower'].isin(_sentences) ]         # select all validated records for that list
    test_df: pd.DataFrame = test_df.drop(columns=['sentence_lower'], errors="ignore")               # drop temp columns
    test_voices: "list[str]" = test_df['client_id'].to_list()
    df_write(test_df, os.path.join(pth, 'test.tsv'))                                                # output the result
    

    # Dev
    _slice: pd.DataFrame = sentences_df[ (sentences_df['cumulative'].astype(int) > test_target) & (sentences_df['cumulative'].astype(int) <= test_target + dev_target ) ]
    ...
    _check_intersection(df1=dev_df, df2=test_df, df1_name='DEV Split', df2_name='TEST Split')

    # Train
    _slice: pd.DataFrame = sentences_df[ sentences_df['cumulative'].astype(int) > test_target + dev_target ]
    _sentences: list[str] = _slice['sentence_lower'].to_list()
    train_df: pd.DataFrame = validated_df[ validated_df['sentence_lower'].isin(_sentences) ]
    ...
    # check any possible intersection(s)
    _check_intersection(df1=train_df, df2=test_df, df1_name='TRAIN Split', df2_name='TEST Split')
    _check_intersection(df1=train_df, df2=dev_df, df1_name='TRAIN Split', df2_name='DEV Split')

    if VERBOSE:
        print(f'>>> Result splits - Train: {train_df.shape[0]} recs, Dev: {dev_df.shape[0]} recs, Test: {test_df.shape[0]} recs ')
        print(f'--- Voice overlaps:')
        print(f'--- TRAIN has voices already in DEV: {len(set(train_voices).intersection(dev_voices))}')
        print(f'--- TRAIN has voices already in TEST: {len(set(train_voices).intersection(test_voices))}')
        print(f'--- DEV has voices already in TEST: {len(set(dev_voices).intersection(test_voices))}')
...
def main() -> None:
    print('=== New Corpora Creator Algorithm Proposal for Common Voice Datasets ===')
    ...
    # Remove old files
    to_delete: "list[str]" = []
    to_delete.extend(glob.glob(os.path.join(dst_exppath, '**', 'train.tsv'), recursive=True))
    to_delete.extend(glob.glob(os.path.join(dst_exppath, '**', 'dev.tsv'), recursive=True))
    to_delete.extend(glob.glob(os.path.join(dst_exppath, '**', 'test.tsv'), recursive=True))
    for fpath in to_delete:
        os.remove(fpath)

    exp_corpora_paths: "list[str]" = glob.glob(os.path.join(dst_exppath, '*'), recursive=False)

    # Get total for progress display
    all_validated: "list[str]" = glob.glob(os.path.join(dst_exppath, '**', 'validated.tsv'), recursive=True)
    ...
    for corpus_path in exp_corpora_paths:
        ...
        for locale_path in exp_corpus_locale_paths:
            ...
            if VERBOSE:
                ...
                print('\033[F' + ' ' * 80)
                print(f'\033[FProcessing {cnt}/{all_validated} => {exp_corpus_name} - {locale_name}')
