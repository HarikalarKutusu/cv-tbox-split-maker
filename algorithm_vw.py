...
TRAIN_PERCENTAGE: float = 90.0
DEV_PERCENTAGE: float = 5.0
TEST_PERCENTAGE: float = 5.0
...
def corpora_creator_vw(val_path: str, dst_path: str):
    ...
    if total_validated >= SAMPLE_SIZE_THRESHOLD:
        ...
        if conf.VERBOSE:
            ...
            print(
                f'>>> TEST : {float("{:.2f}".format(100 * test_target/total_validated))}% => {test_target} recs OR max {test_voice_max} voices'
            )
            print(
                f'>>> DEV  : {float("{:.2f}".format(100 * dev_target/total_validated))}% => {dev_target} recs OR max {dev_voice_max} voices'
            )
            print(
                f'>>> TRAIN: {float("{:.2f}".format(100 * train_target/total_validated))}% => {train_target} recs OR remaining from TEST & DEV'
            )
            # print()
    else:
        ...
        if conf.VERBOSE:
            ...
            print(
                f">>> TEST : {TEST_PERCENTAGE}% => {test_target} recs OR max {test_voice_max} voices"
            )
            print(
    ...
    if (
        test_missing > 0 and test_slice.shape[0] > 5 and train_slice.shape[0] > 5
    ):  # do it only missing & possible
        inx: int = -1
        delta_test: int = 0
        delta_train: int = 0
        limit_reached: bool = False
        while (delta_train - delta_test) < test_missing and not limit_reached:
            inx += 1
            try:
                delta_train += int(
                ...
                delta_test += int(
                    test_slice["recorded_count"].iat[-inx]
        ...
        delta_test_df: pd.DataFrame = test_slice[
            -inx - 1 :
        ]  # Get the tail to move to train (will happen later)
        delta_train_df: pd.DataFrame = train_slice[
            : inx + 1
        ]  # Get the head to move to test
        test_slice: pd.DataFrame = pd.concat(
            [test_slice[: -inx - 1], delta_train_df]
        )  # To head of test, append head of train
        train_slice: pd.DataFrame = train_slice[
            inx + 1 :
    ...
    # Handle DEV-TRAIN
    dev_missing: int = dev_target - (
        actual_dev_target - actual_test_target
    ...
    test_voices: "list[str]" = (
        test_slice["v_enum"].unique().tolist()
    )  # get a list of unique voice enum in test set
    total_test_voices: int = len(
        test_voices
    )  # get a list of unique voice enum in test set
    test_df: pd.DataFrame = validated_df[
        validated_df["v_enum"].isin(test_voices)
    ]  # select all validated records for that list
    total_test_sentences: int = len(
        test_df["s_enum"].unique().tolist()
    )  # get a list of unique voice enum in test set
    ...
    if conf.VERBOSE:
        ...
        print(
            f'--> RESULT VOICES  (TRAIN + DEV + TEST) = {total_train_voices} ({float("{:.2f}".format(100 * total_train_voices/tot_v))}%) + '
        ...
        print(
            f'--> RESULT RECORDS (TRAIN + DEV + TEST) = {train_recs} ({float("{:.2f}".format(100 * train_recs/tot_r))}%) + '
...
def main() -> None:
    """Main loop for vw algorithm for whisper fine-tuning for Common Voice Datasets"""
    ...
    for val_path in all_validated:
        ...
        if lc not in WHISPER_LANGUAGES:
            ...
            corpora_creator_vw(val_path=val_path, dst_path=dst_corpus_dir)
            print()
